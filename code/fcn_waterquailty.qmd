...
####Priors model
```{r}
#| label: tank_samp_rand_model_1_NO3
#| output: true
#| eval: true
#| warning: false
#| message: false
...
WQ.NO3.form <- bf(NO3 ~ treatment + (1|tank_rep_rand:sample), family = Gamma(link = "log"))
                    
get_prior(WQ.NO3.form, data = dislv_WQ) 

dislv_WQ |> group_by(treatment) |> summarise(median(log(NO3)), mad(log(NO3))) 

priors_NO3 <- prior(normal(-0.332, 0.6), class = 'Intercept') + 
  prior(student_t(3, 0, 0.9), class = 'b') + 
  prior(student_t(3, 0, 0.4), class = 'sd') + 
  prior(gamma(2, 1), class = 'shape')

WQ.NO3.brm3 <- brm(WQ.NO3.form, 
                 data = dislv_WQ,
                 prior = priors_NO3, 
                 sample_prior = 'only',
                 iter = 5000, 
...
WQ.NO3.brm3 |> conditional_effects("treatment") |> plot(points = TRUE)   
#OK
```

####With data
```{r}
#| label: tank_samp_rand_model_2_NO3
#| output: true
#| eval: true
#| warning: false
#| message: false
...
WQ.NO3.brm4 <- WQ.NO3.brm3 |> update(sample_prior = "yes",
                                       refresh = 0, 
                                       iter = 10000,
                                       warmup = 2500,
                                       seed = 123) 

WQ.NO3.brm4 |> conditional_effects("treatment") |> plot(points = TRUE, jitter_width = 0.2)
```

####MCMC sampling diagnostics

```{r}
#| label: tank_samp_rand_model_3_NO3
#| output: false
#| eval: true
#| warning: false
#| message: false
#| cache: true 
pars <- WQ.NO3.brm4 |> get_variables()
pars_2 <- str_extract(pars, '^b_.*|^sd_.*|^shape$') |> na.omit() 

#Traceplots
WQ.NO3.brm4$fit |> stan_trace(pars = pars_2, inc_warmup = TRUE)
#good 

#Autocorrelation
WQ.NO3.brm4$fit |> stan_ac(pars = pars_2) 
#good

#Rhat
WQ.NO3.brm4$fit |> stan_rhat()
#good

#ESS
WQ.NO3.brm4$fit |> stan_ess() 
#OK
...
```{r}
#| label: tank_samp_rand_model_4_NO3
#| output: true
#| eval: true 
#| warning: false
#| message: false
...
WQ.NO3.brm4 |> pp_check(type = 'dens_overlay', ndraws = 200) + theme_classic()
#pretty good - bimodal means will not be perfect
```

####DHARMA residuals

```{r}
#| label: tank_samp_rand_model_5_NO3
#| output: true
#| eval: true 
#| warning: false
#| message: false
...
WQ.resids <- make_brms_dharma_res(WQ.NO3.brm4, integerResponse = FALSE)

wrap_elements(~testUniformity(WQ.resids)) + 
  wrap_elements(~plotResiduals(WQ.resids, form = factor(rep(1, nrow(dislv_WQ))))) +
  wrap_elements(~plotResiduals(WQ.resids, quantreg = FALSE)) +
  wrap_elements(~testDispersion(WQ.resids))
#KS triggered but visually is OK - is the bimodal bit in the middle causing a bit of a miss, not much we can do about that 
#residuals vs predicted looks OK, dispersion OK
```

####Model Investigations 
#####Summary
```{r}
#| label: model_investigations_1_NO3
#| output: true
#| eval: true
#| warning: false
#| message: false
...
#####Means plot
```{r}
#| label: model_investigations_2_NO3
#| output: true
#| eval: true 
#| warning: false
#| message: false
...
#make plot that displays mean and HDCI
WQ_NO3_plot <- ggplot(data = WQ.NO3.means, aes(x=factor(treatment, level = level_order_wq), y = response, colour = treatment)) + 
  geom_point(data = dislv_WQ, aes(x = treatment, y = NO3), colour = "black", alpha = 0.2, size = 0.5, position = position_jitter(w = 0.1, h = 0)) +
  geom_pointrange(aes(ymin = lower.HPD, ymax = upper.HPD), fatten = 5) + 
  theme_classic() +
  theme(legend.position = "none", 
        text = element_text(size=15), 
  ...
  scale_x_discrete(expression(Treatment), 
                   breaks = waiver(), 
                   labels = stringr::str_wrap(wq_labels, 12)) + 
  scale_y_continuous(expression(NO[3]~(µmol~L^-1))) + 
  scale_colour_manual(values=group.colors_wq) + 
  ggplot2::annotate(geom = "text", 
                    label = expression(NO[3]), 
                    size = 5, 
                    x = 0.5, 
                    y = Inf, 
                    hjust = 0, 
...
#make plot that displays mean and HDCI
WQ_NO3_plot_new <- ggplot(data = WQ.NO3.means, aes(x=factor(treatment, level = level_order_wq_new), y = response, colour = treatment)) + 
  geom_pointrange(aes(ymin = lower.HPD, ymax = upper.HPD), fatten = 5) + 
  theme_classic() +
  theme(legend.position = "none", 
        text = element_text(size=15, colour = "black"),
...
sTreat <- WQ.NO3.brm4$data |> pull(treatment) |> levels()
STreat <- rep(1, length(sTreat))
names(STreat) <- sTreat

WQ.NO3.brm4 |> emmeans(~treatment) |> 
  gather_emmeans_draws() |>  
  mutate(.value = exp(.value)) |> 
  #filter(.draw == 1, month == 1) |> 
  ungroup() |>
